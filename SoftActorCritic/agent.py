...
class dqn :
    ...
    def learn(self) :
        ...
        states = torch.tensor(states[batch_index], dtype = torch.float32) / 255.0
        rewards = torch.tensor(rewards[batch_index], dtype = torch.float32)
        next_states = torch.tensor(next_states[batch_index], dtype = torch.float32) / 255.0
        dones = torch.tensor(dones[batch_index], dtype = torch.bool)
